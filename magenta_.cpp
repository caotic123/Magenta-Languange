...
int* analyse_cond(std::string cond, std::string cond_ex[_cond_p])
{
    ...
    for (int i = 0; i <= cond.length() - 1; i++) {
        ...
        if (i_c_ex(cond_ex, c_)) {
            if (c_ != cond_ex[0] && c_ != cond_ex[1] && !i_c_ex(cond_ex, cond.substr(i + 1, 1))) {
    ...
    char char_ign[ig__][2])
{
    ...
    for (int i = x; i >= 0; i--) {
        ...
        if ((is_operator(s.substr(i, 1), operators) || (s.substr(i, 1) == s__[0]) || (s.substr(i, 1) == s__[1])) && s.substr(i, 1) != c && p__ == 0) {
    ...
    char char_ign[ig__][2])
{
    ...
    for (int i = x; i <= s.length() - 1; i++) {
        ...
        if ((is_operator(s.substr(i, 1), operators) || (s.substr(i, 1) == s__[1])) && s.substr(i, 1) != c && p__ == 0) {
            p_ = i;
            return p_;
        }
    }
    ...
    char char_ign[ig__][2])
{
    ...
    for (int i_ = 0; i_ <= str_.length(); i_++) {
        ...
        if ((is_operator(c, operators) || c == s__[1] || c == s__[0] || c == "\0") && c != op && b) {
    ...
    std::string str, bool t = true)
{
    ...
    if (str.substr(0, 1) != char__ || str.substr(str.length() - 1, 1) != char____) {
